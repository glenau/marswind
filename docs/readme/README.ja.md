...
- **話しながら翻訳する**-単語は文の終わりを待たず、確定した時点で翻訳へ送られ、訳
  文も一語ずつ届きます
- **モデルをアプリ内で管理**：認識モデル 6 種と翻訳モデル 3 種を、進捗表示と
  SHA-256 検証つきでダウンロード。すべて MIT または Apache-2.0
...
**設定 → このアプリについて → アップデートを確認。** 新しいリリースがあるかを GitHub
に尋ね、あれば「ダウンロード」にイメージを取得し、隣に公開されたチェックサムと照合して
Finder で表示します。導入は初回と同じドラッグです。

自動では何も確認しません。タイマーも起動時の確認もなく、ボタンを押していない
ネットワーク通信は行いません。

自分でビルドしたコピーは、入れたときと同じ方法で更新します。もう一度
`npm run install:macos` を実行してください。
...
- ネットワーク通信は、ボタンを押したときだけです。モデルのダウンロードと更新の確認の
  2 つで、タイマーや起動時の通信はありません。
...
ます。カタログに載るのは、読まずに受け入れられる条件のものだけです。whisper と
Silero のモデルは MIT、Qwen3 は Apache-2.0 です。設定の各行は、ダウンロードが
